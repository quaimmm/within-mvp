          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-7">
              ...
              <section className="mt-10 border-t border-border pt-6">
                <p className="text-[10px] text-muted">Why this needs approval</p>
                <p className="mt-3 text-[13px] leading-6 text-ink">{approval.reviewReason}</p>
              </section>

              <section className="mt-8 border-t border-border pt-6">
                <div className="flex items-center justify-between"><p className="text-[10px] text-muted">Matched rule</p><span className="flex items-center gap-1.5 text-[10px] font-medium text-success"><i className="size-1 rounded-full bg-success" />Rule matched</span></div>
                <p className="mt-4 text-[12px] font-medium text-ink">{approval.ruleName}</p>
                <p className="mt-2 text-[13px] leading-6 text-ink">{approval.ruleDescription}</p>
                <p className="mt-2 text-[9px] text-muted">{approval.category} · {approval.policyId}</p>
              </section>

              <section className="mt-8 border-t border-border pt-6">
                <p className="text-[10px] text-muted">Approval required</p>
                <p className="mt-3 text-[12px] text-ink">{approval.approvalType === "Standard" ? "Standard finance review" : approval.approvalType}</p>
...
function RulesPage({ demoState, setDemoState }: { demoState: DemoState; setDemoState: React.Dispatch<React.SetStateAction<DemoState>> }) {
  ...
  const cadenceLabel = rule?.limitType === "per_transaction" ? "per-transaction" : rule?.limitType;
  const approvalAmount = rule?.approvalThreshold ?? rule?.limitAmount ?? 0;
  const ruleBehaviour = rule?.approvalRequired
    ? `Purchases above £${approvalAmount.toLocaleString("en-GB")} require approval.`
    : rule ? `No human approval is required within the £${rule.limitAmount.toLocaleString("en-GB")} ${cadenceLabel} limit.` : "";
  const activeOnArc = Boolean(rule?.active && rule.settlementGuard?.enforcement === "onchain" && rule.settlementGuard.transactionHash);
  ...
  return (
    <div className="mx-auto max-w-[920px]">
      ...
      {rule && <section className="mt-20 border-t border-border pt-10">
        ...
        <p className="mt-5 text-[11px] leading-5 text-muted">{ruleBehaviour}</p>
